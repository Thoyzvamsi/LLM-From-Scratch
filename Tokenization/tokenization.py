import torch
import pickle
from Tokenization.train_tokenizer import Traintokeinzer

class Tokenization:
    def __init__(self):
        with open(r"data\tokenizer.pkl", "rb") as f:
            data = pickle.load(f)
    ...
    def encode(self,text): # we have to convert given text into unicode then merge repeated pairs 
        ...
        while len(encoded) >= 2:
            stats = Traintokeinzer().get_stat(encoded)
            pair = min(stats ,key=lambda p: self.merges.get(p,float('inf')))
            if pair not in Traintokeinzer().merges:
